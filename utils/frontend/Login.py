...
class Login(Gui):
    def __init__(self, 
        ...
        self.login_username = "tata"
        self.login_password = "1"
    ...
    def login(self):
        ...
        ret, value = imgui.input_text(label="Username", value=self.login_username, buffer_length=20, flags=imgui.INPUT_TEXT_ENTER_RETURNS_TRUE)
        if not ret:
            self.login_username = value 
        
        ret, value = imgui.input_text(label="Password", value=self.login_password, buffer_length=50, flags=imgui.INPUT_TEXT_ENTER_RETURNS_TRUE | imgui.INPUT_TEXT_PASSWORD)
        if not ret:
            ...
            self.string_stream.send(data, self.transport) 
    ...
    def loading(self):
        ...
        if self.is_connecting_to_server:
